...
namespace NInject.Resolver
{
    public static class ComponentLoader
    {
        public static void LoadContainer(IKernel container, string path, string pattern)
        {
    ...
    internal class RegisterComponent : IRegisterComponent
    {
        ...
        public RegisterComponent(IKernel container)
        {
        ...
        public void RegisterType<TFrom, TTo>(bool withInterception = false) where TTo : TFrom
        {
            ...
            {
                //this._container.Bind<TFrom, TTo>();
                this._container.Bind<TFrom>().To<TTo>().InSingletonScope();
            }
        }

        public void RegisterTypeWithControlledLifeTime<TFrom, TTo>(bool withInterception = false) where TTo : TFrom
        {
            //this._container.Bind<TFrom, TTo>(new ContainerControlledLifetimeManager());
            // this._container.Bind<TFrom, TTo>(new );
            // < IProductServices > ().To<ProductServices>()
          //  this._container.Load(Assembly.GetExecutingAssembly());

            //this._container.Bind<TFrom>().To<TTo>().InSingletonScope();
           // this._container.Bind<TFrom>().To<TTo>().InThreadScope();

            //this._container.Bind<TFrom>().ToSelf().InSingletonScope();
        }
    }
}
